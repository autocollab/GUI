    ...
    if room_name in rooms:
        ...
        if username in room.members:
            ...
            if room.multy_connect == True:
                ...
                sio.emit('msg', f'err: Not connected, {data} is privated', room=sid)
...
def disconnect(sid):
    ...
    rom_detail = {
        'multy_connect': rooms[key_room].multy_connect,
        'mode': rooms[key_room].mode,
        'IP': rooms[key_room].IP,
        'ID': rooms[key_room].ID,
        'members': rooms[key_room].members,
    }
    rom_detail = json.dumps(rom_detail)
    sio.emit('infor', f'detail:{rom_detail}', room=key_room)
...
def vehicle_command(sid, data):
    ...
    command = data['data']
    res  = command.split(':')
    if res[0] == 'multy_connect':
        room.multy_connect = res[1] == '1'
        rom_detail = {
            'multy_connect': room.multy_connect,
            'mode': room.mode,
            'IP': room.IP,
        ...
        rom_detail = json.dumps(rom_detail)
        sio.emit('infor', f'detail:{rom_detail}', room=room_name)
        return True
    if res[0] == 'mode':
        room.mode = res[1]
        rom_detail = {
    ...
    sio.emit('vehicle', f'{command}', room = room_name + 'v')
