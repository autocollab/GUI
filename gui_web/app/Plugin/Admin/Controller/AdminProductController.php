...
{
    public $uses = array('Node', 'Product', 'Image', 'Video', 'CategoryLinked', 'Hang', 'Agency', 'Tag', 'Filter', 'FilterLinked', 'FilterItem');
    public $limit = 10;
    ...
    public function ajax_getdata()
    {
        ...
        if ($this->data) {
            ...
            if ($url != '') {
                $url = 'http://crw.vncdata.com/bot/bot_content/test_regular2/?link=' . $url;
    ...
    public function product_add()
    {
        ...
        if ($this->data) {
            ...
            if (!isset($this->data['category_id']) || !is_array($this->data['category_id']) || count($this->data['category_id']) <= 0) {
                $this->alert("Bạn chưa chọn mục lục hiển thị", $this->referer());
                die;
            }
            ...
            if (isset($data_thuoctinh['att2'])) {
                $tmp = array();
                foreach ($data_thuoctinh['att2'] as $key => $v) {
                    $tmp[] = $v;
                }
                $data_product['att2'] = $tmp;
                $data_product['att2'] = json_encode($data_thuoctinh['att2']);
            }
            if (isset($data_thuoctinh['att3'])) {
                $tmp = array();
                foreach ($data_thuoctinh['att3'] as $key => $v) {
                    $tmp[] = $v;
                }
                $data_product['att3'] = $tmp;
                $data_product['att3'] = json_encode($data_thuoctinh['att3']);
            }
            ...
            if (isset($data_node['slug']) && $data_node['slug'] != '') {
            ...
            $data_product['google_map'] = ($data_product['google_map']);
            ...
            $this->redirect($this->get_redirect('product', 'add', $node_id));
            die;
        }
    }
    ...
    public function product_edit($node_id)
    {
        ...
        if ($this->data) {
            ...
            // if (isset($data_product['time_start']))
            //     $data_product['time_start'] = json_encode($data_product['time_start']);
            ...
            $this->redirect($this->get_redirect('product', 'edit', $node_id));
            die;
        }
        ...
        // if ($this->is_valid_json($data['Product']['time_start'])) {
        //     $data['Product']['time_start'] = json_decode($data['Product']['time_start']);
        // }
        $this->data = $data;
    ...
    public function product_edit2($node_id)
    {
        $this->_role('product_edit');
        if ($this->data) {
            // pr($this->data); die;
            ...
            if (isset($data_product['price_off']))
                $data_product['price_off'] = preg_replace('/[^0-9]/', '', $data_product['price_off']);
            // 
            if (isset($data_product['adult_price']))
                $data_product['adult_price'] = preg_replace('/[^0-9]/', '', $data_product['adult_price']);

            if (isset($data_product['youth_price']))
                $data_product['youth_price'] = preg_replace('/[^0-9]/', '', $data_product['youth_price']);

            if (isset($data_product['infant_price']))
                $data_product['infant_price'] = preg_replace('/[^0-9]/', '', $data_product['infant_price']);
            // 
            if (isset($data_product['adult_price_pr']))
                $data_product['adult_price_pr'] = preg_replace('/[^0-9]/', '', $data_product['adult_price_pr']);

            if (isset($data_product['youth_price_pr']))
                $data_product['youth_price_pr'] = preg_replace('/[^0-9]/', '', $data_product['youth_price_pr']);

            if (isset($data_product['infant_price_pr']))
                $data_product['infant_price_pr'] = preg_replace('/[^0-9]/', '', $data_product['infant_price_pr']);
            if (isset($data_product['duration']))
                $data_product['duration'] = preg_replace('/[^0-9]/', '', $data_product['duration']);
            // 
            $check_product = $this->Product->find('first', array(
                'conditions' => array(
                    'Product.node_id' => $node_id
    ...
    public function code_qr($nid)
    {
        ...
        if ($this->data) {
            $seri = $this->data['seri'];
            $this->Baohanh = ClassRegistry::init('Baohanh');
            $check = $this->Baohanh->findBySeri($seri);

            if (is_array($check) && count($check) > 0) {
                $this->Session->setFlash("Số seri đã tồn tại");
            } else {
                $this->Product->id = $data['Product']['id'];
                $this->Product->saveField('seri', $seri);
            }
        }
        $this->set('data', $data);
    }
